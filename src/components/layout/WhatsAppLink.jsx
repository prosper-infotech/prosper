...
export default function WhatsAppLink({ className, iconClassName = 'h-8 w-8', eventCategory = 'Chat Support' }) {
  const { pathname } = useLocation()

  const handleClick = (e) => {
    e.preventDefault()
    window.gtag?.('event', 'click_whatsapp', { event_category: eventCategory })
  ...
  return (
    <a
      ...
      className={className}
    >
      <svg viewBox="0 0 32 32" className={iconClassName} fill="#fff" aria-hidden="true">
