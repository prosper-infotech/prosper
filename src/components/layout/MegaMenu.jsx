...
export default function MegaMenu({ item, onNavigate }) {
  ...
  const columns = useTwoColumns
    ...
    : [item.children]
  const width = useTwoColumns ? 'w-[38rem]' : hasNestedChildren ? 'w-80' : 'w-64'
  ...
  return (
    <motion.div
      ...
      className={`absolute left-0 top-full z-40 pt-3 ${width}`}
    >
                    ...
                    <div className="flex flex-col pb-1.5">
                      {child.children.map((grandchild) => (
                        <Link
                          ...
                          to={grandchild.path}
                          onClick={onNavigate}
                          className="px-5 py-1.5 pl-8 text-xs text-white/60 hover:bg-white/10 hover:text-gold transition-colors"
                        >
                          {grandchild.label}
                        </Link>
                      ))}
